...
export function PricingCards({
    title = "Precios y tiempos reales",
    subtitle = "Lo que ves es lo que pagas. Incluye tasas, legalizaciones y envío a tu domicilio.",
    documentName
}: PricingCardsProps) {
    const [activeTab, setActiveTab] = useState<"completo" | "consulado">("completo");

    const getWhatsappUrl = (planName: string, type: "completo" | "consulado") => {
        const baseMsg = documentName
            ? `Hola, me interesa el plan ${planName} ${type === "completo" ? "Completo" : "Consulado"} para la legalización de ${documentName}.`
            : `Hola, necesito el plan ${planName} ${type === "completo" ? "Completo" : "Consulado"}.`;
        return `${WHATSAPP_URL_BASE}?text=${encodeURIComponent(baseMsg)}`;
    };
        ...
        completo: [
            {
                ...
                href: getWhatsappUrl("Estándar", "completo"),
            ...
            },
            {
                ...
                href: getWhatsappUrl("Urgente", "completo"),
        ...
        consulado: [
            {
                ...
                cta: "Solicitar Legalización",
                href: getWhatsappUrl("Estándar", "consulado"),
            ...
            },
            {
                ...
                cta: "Solicitar Legalización",
                href: getWhatsappUrl("Urgente", "consulado"),
    ...
    return (
        <Section id="pricing" className="bg-[var(--bg)] py-24">
            <Container>
                ...
                {/* Tab Switcher */}
                <div className="flex flex-col items-center justify-center mb-12 gap-5 w-full max-w-md mx-auto px-4">
                    <span className="text-xs md:text-sm font-bold text-neutral-500/80 uppercase tracking-widest text-center">
                        Selecciona qué necesitas:
                    </span>

                    <div className="bg-neutral-100 p-1 rounded-full grid grid-cols-2 relative w-full h-12 md:h-14 shadow-inner">
                        <div
                            className={cn(
                                "absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-full shadow-sm transition-all duration-300 ease-in-out",
                                activeTab === "completo" ? "left-1" : "left-1/2"
                            )}
                        />
                        <button
                            onClick={() => setActiveTab("completo")}
                            className={cn(
                                "relative z-10 flex items-center justify-center text-xs md:text-sm font-bold transition-colors duration-200 rounded-full",
                                activeTab === "completo" ? "text-neutral-900" : "text-neutral-500 hover:text-neutral-700"
                            )}
                        >
                            Documento Nuevo
                        </button>
                        <button
                            ...
                            className={cn(
                                "relative z-10 flex items-center justify-center text-xs md:text-sm font-bold transition-colors duration-200 rounded-full",
                                activeTab === "consulado" ? "text-neutral-900" : "text-neutral-500 hover:text-neutral-700"
                            )}
                        >
                            Solo Consulado
                        </button>
                ...
                <Stagger
                    className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-md md:max-w-4xl mx-auto items-start"
                    staggerDelay={100}
                    key={activeTab} // Force re-render on tab change for animation
                >
                    {currentPricingData[activeTab].map((plan: any) => (
                ...
                <Reveal variant="fade-up" duration="slow" delay={200} className="mt-12 flex justify-center">
                    <div className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 text-amber-900 text-base font-medium text-center shadow-md max-w-4xl backdrop-blur-sm">
                        ...
                        <span>Debido a la situación crítica que atraviesa Cuba, hemos tenido que aumentar temporalmente los precios para mantener el servicio y los tiempos.</span>
                    </div>
                </Reveal>
            </Container>
        </Section >
